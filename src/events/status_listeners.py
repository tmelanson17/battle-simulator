...
class ParalysisListener(Listener[BattleState]):
    ...
    def _handle_speed_reduction(self, pokemon: PokemonState):
        ...
        if not self.speed_reduced:
            ...
            print(f"{pokemon.name}'s speed was reduced due to paralysis! ({self.original_base_speed} -> {new_base_speed})")
    ...
    def _handle_move_prevention(self, event_queue: EventQueue, pokemon: PokemonState):
        ...
        for priority_item in all_events:
            ...
            if (action.__class__.__name__ == "MoveAction" and 
                action.player == self.player and 
                action.src_idx == self.pokemon_idx):
...
class ToxicListener(Listener[BattleState]):
    ...
    def on_event(self, input: BattleState, event_queue: EventQueue):
        ...
        if pokemon.status == Status.TOXIC and not pokemon.fainted:
            # Damage increases each turn: 6.25% * turn_number
            damage = max(1, int(pokemon.hp_max * 0.0625 * self.toxic_counter))
            pokemon.hp = max(0, pokemon.hp - damage)
            print(f"{pokemon.name} is badly poisoned! (-{damage} HP, turn {self.toxic_counter})")
...
class BurnListener(Listener[BattleState]):
    ...
    def on_event(self, input: BattleState, event_queue: EventQueue):
        ...
        elif pokemon.status != Status.BURNED and self.attack_reduced:
    ...
    def _handle_attack_reduction(self, pokemon: PokemonState):
        ...
        if not self.attack_reduced:
            ...
            print(f"{pokemon.name}'s attack was reduced due to burn! ({self.original_base_attack} -> {new_base_attack})")
...
class SleepListener(Listener[BattleState]):
    ...
    def on_event(self, input: BattleState, event_queue: EventQueue):
        ...
        if pokemon.status == Status.SLEEP:
            ...
            if self.sleep_turns_remaining <= 0:
                ...
                print(f"{pokemon.name} is fast asleep! ({self.sleep_turns_remaining} turns left)")
    ...
    def _remove_all_moves(self, event_queue: EventQueue, pokemon: PokemonState):
        ...
        def is_move_from_pokemon(event):
            return (event.__class__.__name__ == "MoveAction" and 
...
class CleanupSwitchoutListeners:
    def __init__(self, player: Player, pokemon_idx: int, listener_manager: ListenerManager):
    ...
    def cleanup_fainted_pokemon(self, pokemon_id: PokemonId):
        ...
        self.listener_manager.remove_listener(pokemon_id, lambda l: isinstance(l, StatusListener))
