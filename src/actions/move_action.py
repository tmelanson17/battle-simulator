...
from src.state.pokestate_defs import Player, Move, Status, Category, get_effectiveness, calculate_damage
...
class MoveAction(Action):
    ...
    def execute(self, game_state: GameState):
        ...
        if move.disabled:
            return
        target = game_state.battle_state.get_opponent(self.player).get_active_mon(self.target_idx)
        ...
        if dex_entry.category != Category.STATUS:
            ...
            for effect in from_move(dex_entry):
                print(f"Effect property: {effect.property}, value: {effect.value}")
                if effect.property.name == "status":
                    ...
                    game_state.event_queue.add_event(
                        ApplyStatusAction(Player.opponent(self.player), self.target_idx, Status(effect.value.lower())),
                        Priority(dex_entry.priority, player.get_active_mon(self.src_idx).speed.current_stat)
                    )